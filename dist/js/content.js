...
exports.STATUSCODE_CHANGE = "STATUSCODE_CHANGE";
exports.RESP_TEXT_CHANGE = "RESP_TEXT_CHANGE";
exports.CONTENT_TYPE_CHANGE = "CONTENT_TYPE_CHANGE";
exports.PAGINATION_CHANGE = "PAGINATION_CHANGE";
exports.UPDATE_MESSAGE = "UPDATE_MESSAGE";
exports.UPDATE_INTERCEPTOR_STATUS = "UPDATE_INTERCEPTOR_STATUS";
exports.FETCH_DATA = "FETCH_DATA";
exports.FETCH_DATA_IN_PROGRESS = "FETCH_DATA_IN_PROGRESS";
...
exports.TOGGLE_LISTENING = "TOGGLE_LISTENING";
exports.INITIALISE_DEFAULTS = "INITIALISE_DEFAULTS";
exports.TOGGLE_SHOW_ADD_REQUEST = "TOGGLE_SHOW_ADD_REQUEST";
exports.CHANGE_URL_TABLE = "CHANGE_URL_TABLE";
function errorNotify(errorMessage, tabId) {
    return { type: exports.ERROR, payload: { errorMessage: errorMessage, tabId: tabId } };
}
exports.errorNotify = errorNotify;
function clearFields(tabId) {
    return { type: exports.CLEAR_REQUESTS, payload: { tabId: tabId } };
}
exports.clearFields = clearFields;
function handleCheckToggle(tabId, reqId, checked) {
    return { type: exports.TOGGLE_CHECKBOX, payload: { tabId: tabId, reqId: reqId, checked: checked } };
}
exports.handleCheckToggle = handleCheckToggle;
function handleStatusCodeChange(value, requestId, tabId) {
    return { type: exports.STATUSCODE_CHANGE, payload: { value: value, requestId: requestId, tabId: tabId } };
}
exports.handleStatusCodeChange = handleStatusCodeChange;
function handleRespTextChange(value, requestId, tabId) {
    return { type: exports.RESP_TEXT_CHANGE, payload: { value: value, requestId: requestId, tabId: tabId } };
}
exports.handleRespTextChange = handleRespTextChange;
function handleContentTypeChange(value, requestId, tabId) {
    return { type: exports.CONTENT_TYPE_CHANGE, payload: { value: value, requestId: requestId, tabId: tabId } };
}
exports.handleContentTypeChange = handleContentTypeChange;
function handlePaginationChange(value, tabId, field) {
    return { type: exports.PAGINATION_CHANGE, payload: { field: field, value: value, tabId: tabId } };
}
exports.handlePaginationChange = handlePaginationChange;
function sendMessageToUI(message, tabId) {
    return { type: exports.UPDATE_MESSAGE, payload: { message: message, tabId: tabId } };
}
exports.sendMessageToUI = sendMessageToUI;
function updateInterceptorStatus(tabId, value) {
    return { type: exports.UPDATE_INTERCEPTOR_STATUS, payload: { tabId: tabId, value: value } };
}
exports.updateInterceptorStatus = updateInterceptorStatus;
function fetchingResponse(tabId, requestId, fetching) {
    return {
        type: exports.FETCH_DATA_IN_PROGRESS,
        payload: {
            tabId: tabId,
            requestId: requestId,
            fetching: fetching
...
exports.fetchingResponse = fetchingResponse;
function fetchResponse(requestDetails, tabId) {
    return {
        type: exports.FETCH_DATA,
        payload: { requestDetails: requestDetails, tabId: tabId }
    };
}
exports.fetchResponse = fetchResponse;
function fetchSuccess(data, requestId, tabId) {
    return { type: exports.FETCH_DATA_SUCCESS, payload: { response: data, requestId: requestId, tabId: tabId } };
}
exports.fetchSuccess = fetchSuccess;
function fetchFailure(error, requestId, tabId) {
    return { type: exports.FETCH_DATA_FAILURE, payload: { error: error, requestId: requestId, tabId: tabId } };
}
exports.fetchFailure = fetchFailure;
function updateRequest(tabId, request) {
    return { type: exports.UPDATE_REQUEST, payload: { tabId: tabId, request: request } };
}
exports.updateRequest = updateRequest;
function toggleListeningRequests(tabId, enabledStatus) {
    return { type: exports.TOGGLE_LISTENING, payload: { tabId: tabId, enabledStatus: enabledStatus } };
}
exports.toggleListeningRequests = toggleListeningRequests;
function initialiseDefaults(currentTab, currentUrl, interceptStatus) {
    return {
        type: exports.INITIALISE_DEFAULTS,
        payload: { currentTab: currentTab, currentUrl: currentUrl, interceptStatus: interceptStatus }
    };
}
exports.initialiseDefaults = initialiseDefaults;
exports.toggleAddRequestForm = function (showAddRuleModal) {
    return {
        type: exports.TOGGLE_SHOW_ADD_REQUEST,
        payload: { showAddRuleModal: showAddRuleModal }
    };
};
exports.handleChangeUrl = function (value, tabId, index) {
    return { type: exports.CHANGE_URL_TABLE, payload: { value: value, tabId: tabId, index: index } };
};
...
var actions_1 = __webpack_require__(/*! ./../actions */ "./actions/index.ts");
var Intercept = (function () {
    function Intercept() {
        var _this = this;
        this.startMessageListener = function () {
            _this.store.ready().then(function () {
                chrome.runtime.onMessage.addListener(function (request) {
                    switch (request.message) {
                        case "CLEAR_DATA":
                            ...
                            break;
                        default:
                            _this.interceptSelected(request.message, request.tabId);
                    }
                });
            });
        };
        this.clearAll = function () {
            _this.runInterceptor({
                requestsToIntercept: [],
                checkedTabRecords: []
            });
        };
        this.interceptSelected = function (message, tabId) {
            var presentState = _this.store.getState().rootReducer.tabRecord[tabId];
            if (!presentState) {
                return;
            }
            ...
            var requestsToIntercept = presentState.requests.filter(function (req) {
                return presentState.checkedReqs[req.requestId] && tabId;
            });
            var checkedReqs = presentState.checkedReqs;
            var storeRequestRecords = presentState.requestRecords;
            var checkedTabRecords = {};
            for (var checkedReqId in checkedReqs) {
            ...
            if (message !== "DISABLE_INTERCEPTOR") {
                if (requestObj.requestsToIntercept.length < 1 ||
                    !requestObj.tabId ||
                    isEmptyObject(checkedTabRecords) ||
                    requestObj.requestsToIntercept.find(function (req) {
                        req.tabId !== requestObj.tabId;
                    })) {
                    return;
                }
            }
            _this.injectScripts(function () {
                if ((message === "INTERCEPT_CHECKED" || message === "PAGE_REFRESHED") &&
                    requestObj.interceptEnabledForTab) {
                    _this.runInterceptor(requestObj);
                    _this.store.dispatch(actions_1.sendMessageToUI("Interception Success!", requestObj.tabId));
                }
                else if (message === "DISABLE_INTERCEPTOR" && !requestObj.interceptEnabledForTab) {
                    _this.disableInterceptor();
                    _this.store.dispatch(actions_1.sendMessageToUI("Interception Disabled!", requestObj.tabId));
                }
            });
        };
        this.removeScriptFromDom = function (querySelector) {
            while (document.querySelectorAll(querySelector).length) {
                var elemToRemove = document.querySelector(querySelector);
                var parentElement = elemToRemove.parentNode;
                parentElement.removeChild(elemToRemove);
            }
        };
        this.runInterceptor = function (selectedReqs) {
            ...
            _this.removeScriptFromDom("#enableInterceptorScript");
        };
        this.disableInterceptor = function () {
            ...
            script.textContent = selectedInterceptCode;
            (document.head || document.documentElement).appendChild(script);
            _this.removeScriptFromDom("#disableInterceptorScript");
        };
        this.injectScripts = function (callback) {
        ...
        this.store = new webext_redux_1.Store({
